...
## mmr reranking
def mmr(query_vector: np.ndarray,
        document_vectors: np.ndarray,
        selected_documents: List[int],
        lambda_: float=1.0,
        topn: int=100) -> Tuple[List[int], List[float]]:
    index2id = {
        i: id_
        for i, id_ in enumerate(selected_documents)
    }
    selected_indices = [i for i in range(len(selected_documents))]
    ...
    candidates_indices = [document for document in selected_indices if best_document_indices != document]

    for _ in range(min(topn-1, len(selected_indices)-1)):
    ...
    results = [
        (index, float(query_document_similarities.reshape(1, -1)[0][index]))
        for index in best_document_indices
    ]
    results.sort(key=lambda x: x[1], reverse=True)
    indices = [index2id[result[0]] for result in results]
