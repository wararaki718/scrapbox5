...
def predictor() -> Generator[SurvivePredictor, None, None]:
    ...
    with patch("project_template.survive.predictor.PassengerVectorizer", return_value=vectorizer):
        with patch("project_template.survive.predictor.SurviveEstimator", return_value=model):
